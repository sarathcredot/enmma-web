...
//                 console.log("report origin", groupedByYear)
...
//     console.log("report", groupedData)
...
import React, { useEffect, useState } from "react";
import axios from "axios";
import Statementlist from "./Statementlist"; // Assuming you have this component for individual statements
import Pagination from "./Pagination";
import Link from "next/link";
import SVG from "@/components/elements/Allsvg";
...
export default function Statement({ style, showItem, showPagination }) {
    ...
    const showLimit = showItem || 2; // Default to 2 items per page
    const paginationItem = 4; // Number of pagination buttons to display
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/financial-statements/allreport`);
                // Assuming the response.data is already in the correct format
                setStatementsData(response.data.res); 
            } catch (error) {
                console.error("Error fetching data:", error);
            }
        };
    ...
    const totalPages = Math.ceil(statementsData.length / showLimit);

    const getPaginatedData = () => {
        const startIndex = (currentPage - 1) * showLimit;
        const endIndex = startIndex + showLimit;
        return statementsData.slice(startIndex, endIndex);
    };


    console.log( "show report", getPaginatedData())
        ...
        <>
            <div>
                {getPaginatedData().length === 0 ? (
                    ...
                    getPaginatedData().map((group) => (
                        <div key={group._id} className="statement-main m-0">
                            <div className="dev-statmentlist container">
                                <h1 className="fw-title">{group._id}</h1>
                                {group.statements?.map(item => (
                                    <div className="state-list" key={item._id}>
                                        <span>{item.year}</span>
                                        <span className="text-nowrap">
